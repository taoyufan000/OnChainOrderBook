...
contract OlpManager is ReentrancyGuard, Governable, IOlpManager {
    ...
    constructor(address _vault, address _usdg, address _olp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        olp = _olp;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "OlpManager: invalid weight");
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minOlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("OlpManager: action not enabled"); }
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _olpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "OlpManager: cooldown duration not yet passed");

        // calculate aum before sellUSDG
        uint256 aumInUsdg = getAumInUsdg(false);
        uint256 olpSupply = IERC20(olp).totalSupply();

        uint256 usdgAmount = _olpAmount.mul(aumInUsdg).div(olpSupply);
